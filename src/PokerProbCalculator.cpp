...
bool PokerProbCalculator::validateCard(std::string card) const{
    ...
    if(map_of_seen_cards.find(card) != map_of_seen_cards.end())                             return false;
...
bool PokerProbCalculator::validateCards(std::vector<std::string> cards) const{
...
void PokerProbCalculator::addFoldedPlayers(std::vector<int> indices){
    ...
    for(int i=0; i<indices.size(); i++){
        if(indices[i] > players_names.size()){
            this->error_status = true;
...
void PokerProbCalculator::printGameStatus() const{
    ...
    for(int i=0; i<players_names.size(); i++)
        if(folded_players[i] == true)
            std::cout<<std::setw(2*players_names[i].length() + 10)<<std::left<<players_names[i] + " (folded)";
        else
            std::cout<<std::setw(2*players_names[i].length())<<std::left<<players_names[i];

    std::cout<<"\n";

    for(int i=0; i<players_hand_cards.size(); i++)
        if(folded_players[i] == true)
            std::cout<<std::setw(2*players_names[i].length() + 4 + 10)<<std::left<<PokerProbCalculator::formattedCard(players_hand_cards[i].first) + ", " + formattedCard(players_hand_cards[i].second);
        else
            std::cout<<std::setw(2*players_names[i].length() + 4)<<std::left<<PokerProbCalculator::formattedCard(players_hand_cards[i].first) + ", " + formattedCard(players_hand_cards[i].second);
